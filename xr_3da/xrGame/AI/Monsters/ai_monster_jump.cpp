...
void CJumping::Execute()
{
	if (entity) {
		...
		VERIFY		(_entity);
		u16 bone_id = dynamic_cast<CKinematics*>(_entity->Visual())->LL_BoneID("bip01_head");
		CBoneInstance &bone = dynamic_cast<CKinematics*>(_entity->Visual())->LL_GetBoneInstance(bone_id);
