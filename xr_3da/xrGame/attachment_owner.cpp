...
void __stdcall AttachmentCallback(CKinematics *tpKinematics)
{
	...
	CKinematics				*kinematics = dynamic_cast<CKinematics*>(game_object->Visual());
...
//		VERIFY								((*I)->ID() != inventory_item->object().ID());
	...
	if (can_attach(inventory_item)) {
		...
		attachable_item->set_bone_id		(dynamic_cast<CKinematics*>(game_object->Visual())->LL_BoneID(attachable_item->bone_name()));
